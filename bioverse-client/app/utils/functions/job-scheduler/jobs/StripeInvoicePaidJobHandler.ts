...
import { addOrRemoveStatusFlags, wipeStatusFlags } from '@/app/utils/database/controller/prescription_subscriptions/prescription_subscriptions';
...
import { getBaseOrderById, updateOrderMetadata } from '@/app/utils/database/controller/orders/orders-api';
...
export class StripeInvoicePaidJobHandler extends BaseJobSchedulerHandler {
    ...
    private async denoteRenewalOrderAsAutoShippedIfApplicable(
        ...
        */
        ...
        if (
            ...
            if (
                ...
                await createNewAutoRenewalJob(
                    mutatedRenewalOrder.customer_uuid //the only purpose of this job right now (June 5 2025) is to send the autoship macro 
                );
        ...
        } // end of 'if lastPriceDataPaidFor.cadence is quarterly/biannual/annually' - so if it's monthly to monthly,  just return the renewal order as is
    ...
    *
    * This method looks for checkins and uses checkin data to determine whether or not to send a script.
    * Then it updates the order data and only sends a script to pharmacy IF the check-in conditions are met. 
    * If checkin conditions are not met, it updates the order's data and triggers the WL_CHECKIN_INCOMPLETE event. 
    *
    */
    private async updateOrderStatusAfterPaidAndSendScript(
        ...
        * UNLESS they are on a monthly subscription, in which case they can get 5 refills without completing a checkin. 
        */
        ...
        if (
            ...
            /*
            *   Handle monthly non-checkin refills!
            *   if they were on a monthly subscription got charged again for the monthly price with no checkins,
            *   we can send up to 5 refills without a checkin (recorded in the base order's metadata). 
            *   ***This metadata gets reset to 0 outside this if statement if they have completed a checkin*** 
            *   We have to make sure the last price paid for was monthly, because if was quarterly, then they should be put in NCI hold with no checkins.
            * 
            *   TODO: the denoteRenewalOrderAsAutoShippedIfApplicable function handles the case where they were previously on a multi-month subscription and DID complete a checkin but NOT dosage selection, but could be refactored to use this kind of logic instead
            * */
            if (renewalOrder.subscription_type === SubscriptionCadency.Monthly) {
                const lastPriceIdPaidFor = //figure out if they were previously on a monthly subscription
                    await getLastInvoiceBeforeRenewal(
                        subscription.stripe_subscription_id,
                        invoiceId
                    );
                if (!lastPriceIdPaidFor) {throw new Error(`${EngineeringQueueNotes.LastPriceIdNotFound} - no last price id paid for in updateOrderStatusAfterPaidAndSendScript`);}
                const lastPriceDataPaidFor = await getVariantIndexByPriceIdV2(renewalOrder.product_href as PRODUCT_HREF, lastPriceIdPaidFor);
                if (!lastPriceDataPaidFor) {throw new Error(`${EngineeringQueueNotes.LastPriceIdNotFound} - no last price data paid for in updateOrderStatusAfterPaidAndSendScript`);}

                const lastPriceWasMonthly: boolean = lastPriceDataPaidFor.cadence === SubscriptionCadency.Monthly;
                const baseOrder = await getBaseOrderById(renewalOrder.original_order_id);

                if (
                    lastPriceWasMonthly && 
                    (
                        !baseOrder?.metadata ||
                        baseOrder?.metadata?.refills_shipped == null ||
                        baseOrder?.metadata?.refills_shipped < 5 //if they have fewer than 5 refills shipped, we can send a script without a checkin 
                    )
                ) { 
                    ...
                    await updateOrderMetadata( //increment the refills shipped count
                        { refills_shipped: (baseOrder?.metadata?.refills_shipped ?? 0) + 1 }, 
                        renewalOrder.original_order_id
                    );
                    await updateRenewalOrder(renewalOrder.id, {
                        autoshipped: true
                    });
                    await wipeStatusFlags(subscription.id);
                    return; //exit the function early because no status flags are needed for this pat
                } //if last price was monthly of they've exhausted their 5 refills, continue to the following code and give them the NO_CHECK_IN_HOLD_CHARGED status flag
            } //end of monthly non-checkinrefill handling
        ...
        } // end of handling cases where there were no checkins 

        //***if they have completed a checkin, reset the metadata's refills shipped count if it exists*** 
        if (renewalOrder.subscription_type === SubscriptionCadency.Monthly) {
            const baseOrder = await getBaseOrderById(renewalOrder.original_order_id);
            if (baseOrder?.metadata && baseOrder?.metadata?.refills_shipped) { //if they have less than 5 refills shipped, we can send a script without a checkin
                await updateOrderMetadata({ refills_shipped: 0 }, renewalOrder.original_order_id);
            }
        }
...
*
* usedAllRefills
* Currently only used for non-weightloss products in the handleNonWeightlossRenewal function. 
*
* */
