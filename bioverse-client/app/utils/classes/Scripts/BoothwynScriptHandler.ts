...
import { generateBoothwynScriptAsync } from '../../functions/prescription-scripts/boothwyn-script-generator';

export class BoothwynScriptHandler extends BaseScriptHandler {
    async sendScript(): Promise<Status> {
        await this.loadPrescriptionData();

        const res = await processAutomaticBoothwynScript(
    ...
    async generateScript(): Promise<Status> {
        try {
            ...
            if (this.patientData && this.scriptOrderData) {
                const script_json = await generateBoothwynScriptAsync(
                    this.patientData.id,
                ...
                if (!priceData || !priceData.cadence) {
                    await forwardOrderToEngineering(
                        ...
                        'Failed to get pricedata in boothwyn script handler'
                    );
                }
            ...
            console.error(
                'Something went wrong converting variant index for boothwynscripthandler',
                error
            );
            await forwardOrderToEngineering(
                this.renewalOrder.renewal_order_id,
