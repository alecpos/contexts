...
import GlutathioneAdvantagesComponent from '@/app/components/intake-v2/pages/glutathione-advantages';
...
export default async function GlutathioneAdvantagesPage({ params, searchParams }: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    return (
        <>
            <GlutathioneAdvantagesComponent/>
        </>
    );
}
