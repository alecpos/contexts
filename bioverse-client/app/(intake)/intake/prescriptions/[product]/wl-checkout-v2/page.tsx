...
import WLCheckoutContainerUpdated from '@/app/components/intake-v2/pages/wl-checkout-updated';
import {
...
export default async function WlCheckoutV2Page({
    searchParams,
    params,
}: CheckoutProps) {
    ...
    const { productData: fetchedProductData, error: ImagePriceError } =
        await fetchProductImageAndPriceData(existing_order_data.product_href);
...
async function fetchData(product: string, searchParams: WLSearchParams) {
    ...
    // const orderData = await getCombinedWeightlossOrderForUser(user_id);
    ...
    const { data: priceData, error: priceDataError } =
        await getPriceVariantTableData(orderData.product_href);
