...
import GoodToGoV2 from '@/app/components/intake-v2/pages/good-to-go';
...
export default async function GoodToGoPage({ params, searchParams }: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    return (
        <>
            <GoodToGoV2 medicationName={params.product} />
        </>
    );
}
