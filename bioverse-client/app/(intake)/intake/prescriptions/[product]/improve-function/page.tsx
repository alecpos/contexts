...
import ImproveFunctionComponent from '@/app/components/intake-v3/pages/improve-function';
...
export default async function ImproveFunctionPage({ params, searchParams }: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    return (
        <>
            <ImproveFunctionComponent />
        </>
    );
}
