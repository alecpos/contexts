...
import SkincarePreDemographic from '@/app/components/intake-v2/pages/skincare-pre-demographic';
...
export default async function GoodNews({ params, searchParams }: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    return (
        <>
            <SkincarePreDemographic />
        </>
    );
}
