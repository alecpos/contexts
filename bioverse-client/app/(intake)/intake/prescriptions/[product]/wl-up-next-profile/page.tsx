...
import WLUpNextProfileComponent from '@/app/components/intake-v2/pages/wl-up-next-profile';
...
export default async function WLUpNextProfilePage({ params, searchParams }: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    return (
        <>
            <WLUpNextProfileComponent />
        </>
    );
}
