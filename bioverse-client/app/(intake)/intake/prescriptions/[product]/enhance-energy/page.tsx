...
import EnhanceEnergyComponent from '@/app/components/intake-v2/pages/enhance-energy';
...
export default async function EnhanceEnergyPage({ params, searchParams }: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    return (
        <>
            <EnhanceEnergyComponent />
        </>
    );
}
