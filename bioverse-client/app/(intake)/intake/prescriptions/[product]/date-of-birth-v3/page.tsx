...
export default async function DOBPageIntake({ params, searchParams }: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    //Pull profile data to pre-populate fields if available.
    const { dob: date_of_birth, error: profilesError } =
        await getUserDateOfBirth(user_id);
    ...
    return (
        <>
            <DateOfBirthSelectionClientComponentV3
