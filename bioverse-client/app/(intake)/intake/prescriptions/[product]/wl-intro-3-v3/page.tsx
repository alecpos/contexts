...
import WLIntroSecondClientComponent from '@/app/components/intake-v2/pages/wl-intro-2';
import WLIntroThirdClientComponent from '@/app/components/intake-v3/pages/wl-intro-3';
...
export default async function WLIntroPageThird({}: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    return (
        <>
            <WLIntroThirdClientComponent user_id={user_id} />
        </>
    );
}
