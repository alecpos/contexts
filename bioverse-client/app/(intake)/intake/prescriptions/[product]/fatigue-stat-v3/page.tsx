...
import FatigueStatComponent from '@/app/components/intake-v3/pages/fatigue-stat-v3';
...
export default async function FatigueStatPage({ params, searchParams }: Props) {
    //Immediately check user session presence
    const user_id = (await readUserSession()).data.session?.user.id!;

    return (
        <>
            <FatigueStatComponent />
        </>
    );
}
