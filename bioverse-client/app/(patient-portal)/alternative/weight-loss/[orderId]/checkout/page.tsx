...
import AlternativeCheckout from '@/app/components/patient-portal/alternative-selection/weight-loss/alternative-checkout/alternative-checkout';
...
export default async function alternativeWeightLossSelectionPage({
    ...
    if (status != Status.Success || !data?.order_data) {
        switch (data.code) {
            case VERIFY_ORDER_DATA_CODES.NO_ORDER:
                return (
                    <div className='w-full items-center bg-[#F7F8F8] flex justify-center min-h-screen '>
    ...
    return (
        <div className='w-full bg-[#F7F8F8] flex justify-center min-h-screen'>
            <AlternativeCheckout order_data={data.order_data!} />
        </div>
    );
}
