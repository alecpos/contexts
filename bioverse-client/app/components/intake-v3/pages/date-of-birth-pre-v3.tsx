...
import { LoadingButtonCustom } from '../../intake-v2/buttons/LoadingButtonCustom';
...
export default function PreSignupDateOfBirthSelectionClientComponentV3({}: StateSelectionProps) {
    const [userDateOfBirth, setUserDateOfBirth] = useState<string>(
        convertDatabaseDoBToComponentFormat(''),
    );
    ...
    const [dob, setDob] = useSessionStorage('dob', '');
    ...
    const pushToNextRouteWithAgeCheck = async () => {
        ...
        if (checkDateOfBirthValid(userDateOfBirth)) {
            ...
            if (checkOverEightteen(userDateOfBirth)) {
                ...
                await continueButtonExitAnimation(150);
                router.push(
                    `/intake/prescriptions/${product_href}/unavailable-age-v3`,
                );
            }
        } else {
    ...
    return (
        <>
            <div
                className={`flex flex-col animate-slideRight mt-1 mt-[1.25rem] md:mt-[48px]`}
            >
                <p
                    className={`inter-h5-question-header mb-[20px] md:mb-[48px]`}
                >
                    To verify your eligibility, please tell us your date of
                    birth.
                </p>

                <p className="intake-v3-form-label mb-1 ">Date of Birthday *</p>
                <FormControl className="flex-1 " variant="outlined">
                    <TextField
                        ...
                        className="intake-v3-form-label  h-[3rem] md:h-[48px] mb-1"
                    />
                    <BioType className="!text-red-400 inter">
                        {errorMessage}
                    </BioType>
                    <div className="flex justify-center  mt-[1.25rem] md:mt-[48px]">
                        <ContinueButtonV3
