...
export default function HSAInformationDialog({
    openHSADialog,
    setOpenHSADialog,
    recommendedPrices,
}: HSAInformationDialogProps) {
    const getSavingsAnswer = () => {
        const priceData = recommendedPrices[2]
            ? recommendedPrices[2]
            : recommendedPrices[1];

        const monthPlanText = recommendedPrices[2]
            ? '6 month plan'
            : '3 month plan';
        return `FSAs and HSAs are funded with pre-tax dollars from your paycheck and exactly how much you save depends on your tax rate. The average savings is 30% based on average state and federal tax rates – so for example, 30% equals $${priceData?.price_data.savings.hsa_savings.toFixed(
            2,
        )} with a ${monthPlanText} ($${priceData?.price_data.product_price.toFixed(
            2,
        )} payable up front).`;
    };
