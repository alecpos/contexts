...
import WLOrderSummary from './order-summary-v3/wl-order-summary';
...
export default function WLCheckoutV2({
    ...
    const test_id = urlParams.get('test_id') ?? 'none';
    ...
    const url = useParams();
    const { product_href: urlProductHref } = getIntakeURLParams(
        url,
        searchParams
    );

    var product_href = urlProductHref;

    if (urlProductHref === 'weight-loss') {
    ...
    const renderTermsAndConditions = (cadency: string) => {
        if (cadency === 'monthly') {
            return (
                <>
                    <BioType
                        className={`${INTAKE_INPUT_TAILWIND} text-xs pt-2 pb-2 !text-[#00000099]`}
                    >
            ...
            if (variantPriceData.cadence === 'quarterly') {
                return (
                    <>
                        <BioType
                            className={`${INTAKE_INPUT_TAILWIND} text-xs pt-2 pb-3 !text-[#00000099]`}
                        >
                            Prescription products require an evaluation with a
                            licensed medical professional who will determine if
                            a prescription is appropriate.
                        </BioType>
                        <BioType
                            className={`${INTAKE_INPUT_TAILWIND} text-xs pb-3 !text-[#00000099]`}
                        >
                            By clicking &apos;Save payment details,&apos; you
                            agree to our Terms of Service and Privacy Policy. If
                ...
                return (
                    <>
                        ...
                        >
                            ...
                            for your initial{' '}
                            {variantPriceData.cadence === 'biannually'
                                ? '180-day supply, which we expect to last six months.'
                                : '360-day supply, which we expect to last twelve months.'}
                        </BioType>
                        <BioType
                            className={`${INTAKE_INPUT_TAILWIND} text-xs  pb-3 text-[#00000099]`}
                        >
                ...
                return (
                    <>
                        <BioType
                            className={`${INTAKE_INPUT_TAILWIND}  text-xs mt-4  !text-[#00000099]`}
                        >
                            By clicking &apos;Save payment details,&apos; you
                            agree to our Terms of Service and Privacy Policy. If
                            prescribed, you are purchasing an automatically
            ...
            .then(async function (result) {
                ...
                if (result.setupIntent?.status === 'succeeded') {
                    ...
                    if (order_update_status === 'success') {
                        ...
                        //if we are not in an 'ID-after-checkout' test, we assume the user already went through the ID screens:
                        if (
                        ...
                        //if we are in an 'ID-after-checkout' test, we create a job and push to the next route for the user to verify their ID:
                        if (
                            (searchParams.get('id_test') ===
                                AB_TESTS_IDS.WL_SHOW_ID_AFTER_CHECKOUT) 
                        ) {
                            /**
                             * creates a job to be run 10 min from now which checks if the user has uploaded their ID and selfie
                             * if at that point they haven't uploaded both of those, it will tag them ID/Docs and trigger comms
                             */
                            ...
                            router.push(
                                `/intake/prescriptions/${product_href}/pre-id-v3?${search}&${query.toString()}`
                            );
                        } else {
    ...
    const displayPaymentScreen = () => {
        return (
            <div className='flex flex-col'>
                {applePayVisible && (
                    <BioType
                        className={`intake-subtitle mt-2 text-[19px] mb-1`}
                    >
                ...
                */}
                ...
                <BioType
                    className={`${INTAKE_PAGE_BODY_TAILWIND} text-xs text-[#666666] mt-[12px] mb-[10px]`}
                >
                    {displayCitation()}
                </BioType>
            </div>
    ...
    if (showLoadingScreen) {
        return (
            <div className='flex flex-col w-full mt-10 gap-y-4'>
                ...
                <div className='flex justify-center'>
                    <BioType className='body1'>
                        We&apos;re searching our network for your medical
                        provider
                    </BioType>
                </div>
            </div>
    ...
    return (
        <>
            <div
                className={`flex justify-center animate-slideRight w-full md:w-full`}
            >
                <div className='flex justify-center w-full'>
                    <div className='flex flex-col gap-4 w-full md:max-w-[490px]'>
