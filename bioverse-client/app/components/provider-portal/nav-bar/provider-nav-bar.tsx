...
import DoseSpotNotificationChip from './dose-spot-notifications/dose-spot-notification-chip';
...
import ProviderResourcesButton from './provider-resources/provider-resources-button';
...
export default function ProviderNavBar({ role }: Props) {
    ...
    useEffect(() => {
        if (swr_loading || swr_validating) {
            return; // Don't do anything while loading or validating
        }

        // Only proceed if we have data (either null or valid session)
        if (swr_data === undefined) {
            return; // Don't redirect if we don't have data yet
        }

        if (swr_data?.data.session === null) {
            setUserId(undefined);
            router.push(
                `/provider-auth/login?originalRef=${encodeURIComponent(
                    fullPath
                )}`
            );
        } else if (swr_data?.data.session) {
            setUserId(swr_data.data.session.user.id);
        }
    }, [swr_data, swr_loading, swr_validating, router, fullPath]);

    // // Use the new session timeout hook
    // useSessionTimeout({
    //     userId: userId!,
    //     isAdmin: role === BV_AUTH_TYPE.ADMIN,
    //     onTimeout: () => {
        ...
        >
            <div className='flex flex-row items-center ml-[0.83vw]'>
                <Link href={'/'} className='relative font-normal'>
                    <div>
                        <div className='flex  justify-center w-[8.3rem] md:w-[134px] ml-1'>
                            <svg
                                xmlns='http://www.w3.org/2000/svg'
                                className='w-[8.3rem] md:w-[134px]'
                                viewBox='0 0 134 23'
                                fill='none'
                            >
                                <path
            ...
            {userId && (
                <div className='flex flex-row items-center pr-10 gap-[1.5em]'>
                    <DoseSpotNotificationChip />
                    <ProviderTabs userId={userId!} role={role} />
                    <StartSessionButton userId={userId} role={role} />
                    <ProviderResourcesButton userId={userId} role={role} />
                </div>
            )}
        </div>
    );
