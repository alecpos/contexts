...
import PatientThreadMessageTipTap from './components/message-text-display';
import { ThreadMessageObject } from '@/app/types/messages/messages-types';
...
export default function ThreadMessage({
    ...
    const isFromCurrentUser = message.sender_id === user_id;
    ...
    return (
        <>
            <div className='px-4 mt-1'>
                {/* Header with name and timestamp - only shown at the start of a message group */}
                {startsNewGroup && (
                    <div
                    ...
                    >
                        <BioType className='font-interlight text-sm !text-strong font-normal'>
                            {`${message.sender.first_name} ${message.sender.last_name}`}
                            {message.sender.first_name === 'Bobby' &&
                                message.sender.last_name === 'Desai' &&
                                ' M.D.'}
                            {`, ${displayUserTitle(
                                message.sender?.provider_data?.role
                            )}`}
                        </BioType>
                        <BioType className='text-sm text-[#333333BF] font-interlight'>
                            {formatTimestamp(message.created_at)}
                        </BioType>
                    </div>
                )}

                {/* Message content with avatar at the end of the group */}
                <div
                    className={`flex w-full items-end ${
                        isFromCurrentUser ? 'justify-end' : 'justify-start'
                    }`}
                >
                    {/* Avatar for other users (left side) or empty space placeholder */}
                    {!isFromCurrentUser && (
                        <div className='flex-shrink-0 mr-2 w-10'>
                            {endsGroup ? (
                                <Avatar
                                    alt={`${message.sender.first_name.charAt(
                                        0
                                    )}${message.sender.last_name.charAt(0)}`}
                                    src={
                                        message.sender.provider_data
                                            ?.profile_picture_url || ''
                                    }
                                >
                    ...
                    {/* Message bubble */}
                    <div
                        className={`max-w-4xl px-4 py-2 h-full ${
                            isFromCurrentUser
                                ? endsGroup
                                    ? 'bg-[#D7E3EB] rounded-t-lg rounded-l-lg'
                                    : 'bg-[#D7E3EB] rounded-2xl'
                                : endsGroup
                                ? 'bg-[#DCDCDC] rounded-r-lg rounded-t-lg bubble-grey-border'
                                : 'bg-[#DCDCDC] rounded-2xl bubble-grey-border'
                        }`}
                    >
                        <span className='text-sm text-[#1B1B1B] flex flex-col h-full'>
                            {message.content && message.content.length > 0 && (
                                <PatientThreadMessageTipTap
                    ...
                    {/* Avatar for current user (right side) or empty space placeholder */}
                    {isFromCurrentUser && (
