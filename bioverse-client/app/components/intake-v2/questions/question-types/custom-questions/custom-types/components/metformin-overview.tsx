'use client';

import React, { useState } from 'react';
import BioType from '../../../../../../global-components/bioverse-typography/bio-type/bio-type';
import {
    INTAKE_PAGE_BODY_TAILWIND,
    INTAKE_PAGE_HEADER_TAILWIND,
    INTAKE_PAGE_SUBTITLE_TAILWIND,
} from '../../../../../styles/intake-tailwind-declarations';
import { Button, CircularProgress, Divider } from '@mui/material';

interface OverviewComponentProps {
    handleContinueButton: any;
    isButtonLoading: boolean;
}

export default function MetforminOverviewComponent({
    handleContinueButton,
    isButtonLoading,
}: OverviewComponentProps) {
    return (
        <>
            <div className={`justify-center flex`}>
                <div className='flex flex-col gap-8'>
                    <div className='flex flex-col md:gap-[28px] gap-4 animate-slideRight'>
                        <BioType className={`${INTAKE_PAGE_HEADER_TAILWIND}`}>
                            Metformin
                        </BioType>
                        <div className='flex flex-col gap-4'>
                            <div className='flex flex-col md:gap-2 gap-4'>
                                <BioType
                                    className={`${INTAKE_PAGE_SUBTITLE_TAILWIND}`}
                                >
                                    Overview
                                </BioType>
                                <BioType
                                    className={`${INTAKE_PAGE_BODY_TAILWIND} text-textSecondary`}
                                >
                                    Metformin is a widely prescribed medication
                                    primarily for Type 2 diabetes, known for its
                                    effectiveness in improving insulin
                                    sensitivity and reducing blood sugar levels.
                                    <br />
                                    <br />
                                    Additionally, metformin has shown potential
                                    in aiding weight management and improving
                                    cardiovascular health, making it a valuable
                                    tool in addressing metabolic disorders.
                                </BioType>
                            </div>
                            <Divider />
                            <div className='flex flex-col md:gap-2 gap-4'>
                                <BioType
                                    className={`${INTAKE_PAGE_SUBTITLE_TAILWIND}`}
                                >
                                    Results
                                </BioType>
                                <BioType
                                    className={`${INTAKE_PAGE_BODY_TAILWIND} text-textSecondary`}
                                >
                                    Studies have shown that patients taking
                                    metformin experienced a 5% decrease in body
                                    weight over one year.*
                                </BioType>
                            </div>
                            <Divider />
                            <div className='flex flex-col md:gap-2 gap-4'>
                                <BioType
                                    className={`${INTAKE_PAGE_SUBTITLE_TAILWIND}`}
                                >
                                    Cost
                                </BioType>
                                <BioType
                                    className={`${INTAKE_PAGE_BODY_TAILWIND} text-textSecondary`}
                                >
                                    BIOVERSE offers metformin for as little as $5 for the first month – much more affordable than
                                    other healthcare providers. Unlike other
                                    healthcare providers, we do not require
                                    insurance or prior authorization. Your
                                    medication would be shipped to your home at
                                    no additional cost.
                                </BioType>
                            </div>
                        </div>
                        <div className='md:my-0 my-4'>
                            <Button
                                variant='contained'
                                fullWidth
                                sx={{
                                    width: {
                                        xs: 'calc(100vw - 48px)',
                                        sm: '100%',
                                    },
                                    height: '52px',
                                    // '@media (min-width:768px)': {
                                    //     width: '118px',
                                    // },
                                    // position: { xs: 'fixed', sm: 'static' },
                                    // bottom: { xs: bottomXs, sm: 0 },
                                    padding: '16px',
                                    zIndex: 30,
                                    backgroundColor: '#000000',
                                    '&:hover': {
                                        backgroundColor: '#666666',
                                    },
                                }}
                                onClick={handleContinueButton}
                            >
                                <BioType className='hidden md:flex'>
                                    {isButtonLoading ? (
                                        <CircularProgress
                                            sx={{ color: '#FFFFFF' }}
                                        />
                                    ) : (
                                        'Select as preferred medication and continue'
                                    )}
                                </BioType>
                                <BioType className='md:hidden'>
                                    {isButtonLoading ? (
                                        <CircularProgress
                                            sx={{ color: '#FFFFFF' }}
                                        />
                                    ) : (
                                        'Select as preferred medication'
                                    )}
                                </BioType>
                            </Button>
                        </div>
                        <div className='flex flex-col gap-2 md:mb-0 mb-[100px]'>
                            <BioType
                                className={`${INTAKE_PAGE_BODY_TAILWIND} opacity-[.48] `}
                            >
                              
                                * When used in combination with a
                                restricted calorie diet and exercise program.
                            </BioType>
                        </div>
                    </div>
                </div>
            </div>
        </>
    );
}
