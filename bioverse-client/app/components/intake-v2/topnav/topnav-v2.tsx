...
import { NAD_INJECTION_PRODUCT_HREF } from '@/app/services/tracking/constants';
...
import { getCurrentIntakeProgressBySection } from '@/app/utils/functions/intake-route-controller';
...
export default function NewIntakeTopNav({ logged_in }: TopNavProps) {
    const [question_length, setQuestionLength] = useState<number>(0);
    const [currentProgress, setCurrentProgress] = useState<number>(0);
    const [currentProgressSection, setCurrentProgressSection] =
        useState<number>(1);
    ...
    useEffect(() => {
        const values = calculateProgress();
        setCurrentProgress(values.progress);
        setCurrentProgressSection(values.currentSection);
    ...
    const BorderLinearProgress = styled(LinearProgress)(({}) => ({
        ...
        [`& .${linearProgressClasses.bar}`]: {
            borderRadius: 5,
            background: `linear-gradient(to right, #3B8DC5 0%, #59B7C1 100%)`,
        },
    }));

    /**
     * For progress bars, segment math comes down to:
     * 500 px for the entire thing
     *
     * 40 px for inactive segments = 80 px
     * 12 px gap between each segment = 24 px
     * active segment = 396 px
     *
     */
    ...
    const calculateProgress = (): {
        ...
        const values = getCurrentIntakeProgressBySection(
            pathName,
            params.product as string
        );

        console.log('intake bar vals:', values);

        return values;
    };

    const determineProgressBasedOnSection = (
        target_section: number
    ): number => {
        if (currentProgressSection < target_section) {
            return 0;
        }
    ...
    //for sem-sl banner spring sale test
    const mobileLogoWidth = searchParams.get('banner_id') === AB_TESTS_IDS.SEM_SL 
                                    ? 'w-[5.38rem]' 
                                    : 'w-[8.3rem]';
    ...
    const renderLogo = () => {
        if (
            ...
            pathName.includes('semaglutide') ||
            pathName.includes('tirzepatide') ||
            pathName.includes('weight-loss') ||
            pathName.includes('b12-injection') ||
            pathName.includes('nad-injection') ||
            pathName.includes('sermorelin')
        ) {
            return (
                <div className={`flex justify-center ${mobileLogoWidth} md:w-[134px]`}>
                    <svg
                        xmlns='http://www.w3.org/2000/svg'
                        className={`${mobileLogoWidth} md:w-[134px]`}
                        viewBox='0 0 134 23'
                        fill='none'
                    >
                        <path
        ...
        if (pathArray.includes('new-checkout')) {
            return null;
        }

        // if (pathName.includes(PRODUCT_HREF.RUSH_MELTS)) {
        //     return (
    ...
    const RUSH_MELTS_EXCLUDE = ['/rush-melts/ed-checkout'];

    if (
        RUSH_MELTS_EXCLUDE.some((excludedPath) =>
    ...
    //for sem-sl banner spring sale test
    const mobileMarginBottom = searchParams.get('banner_id') === AB_TESTS_IDS.SEM_SL 
        ? 'mb-[0.64rem] md:mb-[15.77px]' 
        : 'mb-[15.77px]';

    return (
        <>
            <div className={`flex flex-col items-center md:max-w-[520px] w-[90%] mt-8 gap-6 ${mobileMarginBottom}`}>
                {currentProgress !== -1 && (
                    <div className='flex flex-row gap-[12px] max-w-[500px] w-full'>
                        <BorderLinearProgress
                            variant='determinate'
                            value={determineProgressBasedOnSection(0)}
                            color='primary'
                            sx={{
                                width: {
                                    xs: `${
                                        currentProgressSection === 0
                                            ? '90%'
                                            : '8%'
                                    }`,
                                    sm: `${
                        ...
                        <BorderLinearProgress
                            variant='determinate'
                            value={determineProgressBasedOnSection(1)}
                            color='primary'
                            sx={{
                                width: {
                                    xs: `${
                                        currentProgressSection === 1
                                            ? '90%'
                                            : '8%'
                                    }`,
                                    sm: `${
                        ...
                        <BorderLinearProgress
                            variant='determinate'
                            value={determineProgressBasedOnSection(2)}
                            color='primary'
                            sx={{
                                width: {
                                    xs: `${
                                        currentProgressSection === 2
                                            ? '90%'
                                            : '8%'
                                    }`,
                                    sm: `${
