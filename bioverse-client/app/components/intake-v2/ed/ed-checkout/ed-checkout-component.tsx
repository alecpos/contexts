...
import { updateOrderAfterCardDown } from '@/app/utils/database/controller/orders/orders-api';
import {
    getCustomerStripeId,
...
import EDOrderSummary from './ed-order-summary';
import {
    EDCadenceData,
    EDSelectionMetadata,
...
export default function EDCheckoutComponent({
    session_id,
    userEmail,
    userProfileData,
    setupIntentId,
    ...
    //Promo codes are not programmed yet.
    ...
    const [selectedCadence, setSelectedCadence] = useState<
        EDCadenceData | undefined
    >(priceData.find((item) => item.cadence === 'quarterly') ?? priceData[0]);

    const changeSelectedCadence = (newCadenceData: EDCadenceData) => {
        setSelectedCadence(newCadenceData);
    };
    ...
    const renderTermsAndConditions = (cadency: string) => {
        return (
            <>
                <BioType className="body2 pt-2 pb-2 !text-[#00000099]">
                ...
                <BioType className="body2 pb-2 text-[#00000099]">
                    ...
                    {selectedCadence?.price_data.product_price} every{' '}
                    {mapCadencyToDays(cadency)} days until you cancel.
                </BioType>
            ...
            .then(async function (result) {
                ...
                if (result.setupIntent?.status === 'succeeded') {
                    ...
                    const order_update_status = await updateOrderAfterCardDown(
                        ...
                        orderData,
                        {
                            product_href: selectedCadence?.product_href,
                            variant_index: selectedCadence?.variant_index,
                            price_id:
                                selectedCadence?.stripe_price_ids[
                                    process.env.NEXT_PUBLIC_ENVIRONMENT!
                                ],
                            subscription_type: selectedCadence?.cadence,
                            variant_text: selectedCadence?.variant,
                            price: selectedCadence?.price_data.product_price,
                        },
                    );
                    ...
                    if (order_update_status === 'success') {
                        ...
                        await triggerOrderConfirmCommsEvent(
                            session_id,
                            selectedCadence!.product_href,
                        );

                        const eventId = `${currentOrderId}-${Date.now()}`;

                        await triggerEvent(session_id, ORDER_RECEIVED, {
                            product: selectedCadence!.product_href,
                            subscriptionType: selectedCadence!.cadence,
                            variant: selectedCadence!.variant,
                            estimated_total:
                                selectedCadence?.price_data.product_price,
                            subscriptionPrice:
                                parseFloat(
                                    selectedCadence?.price_data.product_price,
                                ).toFixed(2) || '',
                            order_id: currentOrderId,
                            value: selectedCadence?.price_data.product_price,
                            currency: 'USD',
                            context: {
                                event_id: eventId,
                                fbc,
                            ...
                            event_time: Math.floor(Date.now() / 1000),
                            custom_data: {
                                value: selectedCadence?.price_data
                                    .product_price,
                                currency: 'USD',
                            },
                        });
                        ...
                        const payload_meta = {
                            ...
                            content_name: selectedCadence!.product_href,
                        ...
                        if (gclid) {
                            await insertNewGclid(
                                session_id,
                                selectedCadence?.product_href || 'unknown',
                                gclid,
                            );
                        }
                        ...
                        const payload_mixpanel = {
                            order_id: currentOrderId,
                            product_name: selectedCadence!.product_href,
                            subscriptionType: selectedCadence!.cadence,
                        };
                        ...
                        const query = new URLSearchParams({
                            pn: mapProductToCode(selectedCadence!.product_href),
                        }).toString();
                        const url = `/intake-complete/confirmation?${query}&orderId=${currentOrderId}`;
                        router.push(url);
                    }
                }
            });
    ...
    const displayPaymentScreen = () => {
        return (
            <div className="flex flex-col">
                ...
                {renderTermsAndConditions(selectedCadence?.cadence)}
    ...
    return (
        <>
            <div
                className={`flex justify-center overflow-hidden animate-slideRight`}
            >
                <div className="flex justify-center w-full md:mx-0">
                    <div className="flex flex-col gap-4">
                        <div className="flex w-full flex-col gap-4">
                            <EDOrderSummary
                                cadenceData={selectedCadence!}
                                priceData={priceData}
