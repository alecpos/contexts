...
export default function CheckoutV2({
            ...
            return (
                <>
                    ...
                    <BioType className="body2 pb-2 text-[#00000099]">
                        ...
                        . You also agree that, if prescribed, you will be
                        charged ${pricingStructure.total_price} for your first{' '}
                        {mapCadencyToDays(cadency)} days supply and $
                        {pricingStructure.item_price} every{' '}
                        {mapCadencyToDays(cadency)} days thereafter until you
                        cancel.
                    </BioType>

                    <BioType className="body2 pb-2 text-[#00000099]">
                        Ongoing shipments may be charged and shipped up to 2
                        days early to accommodate holidays or other operational
                        reasons to support treatment continuity.
                    </BioType>

                    <BioType className="body2 pb-2 text-[#00000099]">
                        Your subscription will renew unless you cancel at least
                        2 days before the next processing date. You can view
                        your processing date and cancel your subscription(s)
                        through your online account or by contacting customer
                        support at support@gobioverse.com.
                    </BioType>
                </>
            );
        }
    ...
    const handleSubmit = async (e: { preventDefault: () => void }) => {
        ...
        stripe
            /**
             * confirm Setup method is the correct, more modern way of confirming a setup intent.
             * We however use the card element which is outdated. Card Element is used because it is the only way to style the
             * Input menu to be all on one line.
             */
            // .confirmSetup({
            //     elements,
            //     confirmParams: {
            ...
            //             billing_details: {
            //                 address: {
            //                     postal_code: userProfileData.zip,
            //                     country: 'US',
            //                 },
            //             },
            //         },
                ...
                if (result.setupIntent?.status === 'succeeded') {
                    ...
                    if (order_update_status === 'success') {
                        ...
                        const product = priceData.find(
                            (record) =>
                                (record.variant_index = product_data.variant),
                        );
...
// const retrieveCustomer = async (stripeId: string) => {
//   const requestBody = {
//     customerId: stripeId,
//   };

//   const response = await fetch("/api/v1/stripe/customer/retrieve", {
//     method: "POST",
//     headers: {
...
//     updateStripeCustomerId(data.id, sessionData.user.id);
//     setStripeCustomerData(data);
//   }
// };

// const updateSetupIntent = async (customerId: string) => {
//   const payload = {
//     setupIntentId: setupIntentId,
//     customer_id: customerId,
//   };

//   const response = await fetch("/api/stripe/setup-intent/update", {
//     method: "POST",
//     headers: {
//       "Content-Type": "application/json",
//     },
