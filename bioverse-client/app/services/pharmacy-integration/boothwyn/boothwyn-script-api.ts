...
export async function sendBoothwynScript(
    ...
    console.log('SEND SCRIPT BOOTHWYNN DATA: ', script);
    ...
    if (chargeStatus.result === 'success') {
        if (order_type === OrderType.RenewalOrder) {
            ...
            if (!details.isPaid) {
                ...
                await forwardOrderToEngineering(
                    renewalOrderId!,
                    null,
                    'Attempted to send Boothwyn renewal order with issue'
                );
        ...
        const isTest = process.env.NEXT_PUBLIC_ENVIRONMENT === 'dev';

        const url = new URL(`${apiUrl}/api/boothwyn/send-script`);
        if (isTest) {
...
export async function processAutomaticBoothwynScript(
    script: BoothwynScriptJSON,
    orderId: string, // renewal order's ID, not renewal_order_id
    providerId: string,
    order_status: string,
    ...
    try {
        ...
        if (res.status === 200) {
            ...
            console.log('error in boothwyn script: ', res);
        }
    } catch (error) {
        console.error(
